...
@Component({
  selector: 'app-admin-competition-search',
  templateUrl: './admin-competition-search.component.html',
  styleUrls: ['./admin-competition-search.component.scss']
})
export class AdminCompetitionSearchComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private authProvider: AuthService,public snackBar: MatSnackBar,) {
    ...
    public OnSearch(){
      this.loading = true;
      this.authProvider.searchCompetitorUsers(this.viewsForm.value).subscribe((data) => {
          this.loading = false;
          this.dtData = data;
